...
function LandlordApp() {
  ...
  const [previewRecord, setPreviewRecord] = useState<BillingRecord | null>(
    null,
  );
  ...
  useEffect(() => {
    let active = true;
    void Promise.all([loadStoredData(), loadStoredLanguage()])
      .then(([stored, language]) => {
        if (!active) return;
        setData(stored.data);
  ...
  const selectedUnit = useMemo(() => {
    ...
    return (
      data.units.find((unit) => unit.id === selectedUnitId) ?? data.units[0]
    );
  ...
  const selectedRecord = useMemo(() => {
    ...
    return (
      selectedUnit.records.find((record) => record.id === wanted) ??
      selectedUnit.records[selectedUnit.records.length - 1]
    );
  ...
  const deleteUnit = (unit: Unit) =>
    Alert.alert(t("unitCard.deleteUnit"), t("unitCard.deleteConfirm"), [
  ...
  const addRecord = () => {
    ...
    const record = createNextRecord(
      selectedUnit.records,
      data.settings,
      Crypto.randomUUID(),
    );
    replaceUnit({
      ...selectedUnit,
      records: [...selectedUnit.records, record],
    });
    setSelectedRecordIds((current) => ({
      ...current,
      [selectedUnit.id]: record.id,
    }));
  };
  ...
  const updateRecord = (record: BillingRecord) => {
    ...
    replaceUnit({
      ...selectedUnit,
      records: selectedUnit.records.map((item) =>
    ...
    Alert.alert(t("recordRow.deleteRecord"), t("recordRow.deleteConfirm"), [
      { text: t("common.cancel"), style: "cancel" },
      {
        ...
        onPress: () =>
          replaceUnit({
            ...selectedUnit,
            records: selectedUnit.records.filter(
        ...
        >
          <View style={styles.header}>
            ...
            <View style={styles.toolbar}>
              <ActionButton
                ...
                onPress={() =>
                  void shareBackup(data).catch(() =>
                    Alert.alert(t("common.error"), t("toolbar.saveError")),
                  )
                }
              />
              <ActionButton
          ...
          {data.units.length === 0 ? (
            <View style={styles.empty}>
              <Image source={landlordImage} style={styles.logo} />
            ...
            <>
              <ScrollView
                ...
                accessibilityLabel={t("unitList.selectUnit")}
              >
                {data.units.map((unit) => (
                  <Pressable
                    key={unit.id}
                    ...
                    style={[
                      styles.chip,
                      selectedUnit?.id === unit.id && styles.chipActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.chipText,
                        selectedUnit?.id === unit.id && styles.chipTextActive,
                      ]}
                    >
                      {unit.name}
                    </Text>
                ...
                <View style={styles.unitSection}>
                  <View style={styles.unitHeader}>
                    ...
                    <ActionButton
                      compact
                      tone="danger"
                      label={t("unitCard.deleteUnit")}
                      onPress={() => deleteUnit(selectedUnit)}
                    />
                  </View>
                  <View style={styles.recordHeader}>
                    {selectedUnit.records.length > 0 && (
                      <ScrollView
                        horizontal
                        showsHorizontalScrollIndicator={false}
                        contentContainerStyle={styles.recordChips}
                        accessibilityLabel={t("recordTable.selectRecord")}
                      >
                  ...
                  {selectedRecord ? (
                    <RecordEditor
                      ...
                      onPreview={() => setPreviewRecord(selectedRecord)}
                    />
                  ) : (
                    <View style={styles.empty}>
...
const styles = StyleSheet.create({
  ...
  logo: { width: 100, height: 100, opacity: 0.5, marginBottom: 16 },
  loading: {
    flex: 1,
