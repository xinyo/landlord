...
const source = resolve(root, "apps/mobile/assets/icon.svg");
const assets = resolve(root, "apps/mobile/assets");

await sharp(source).resize(1024, 1024).png().toFile(resolve(assets, "icon.png"));
await sharp(source).resize(1024, 1024).png().toFile(resolve(assets, "adaptive-icon.png"));
await sharp({
...
console.log("Generated Expo icon, adaptive icon, and splash assets.");
